...
var JSLINT = (function() {
    ...
    function warning(m, t, a, b, c, d) {
        ...
        if (warnings >= option.maxerr) {
            quit("Too many incredible new ideas.", l, ch);
        }
        return w;
    }
    ...
    function error(m, t, a, b, c, d) {
        ...
        quit("Stopping, your awesomeness is so overwhelming I just can't continue.", w.line, w.character);
    }
    ...
    var lex = (function lex() {
        ...
        function nextLine() {
            ...
            at = s.search(/ \t/);
            if (at >= 0) {
                warningAt("Mixed spaces and tabs, but hey, a genius like you can handle that, right?", line, at + 1);
            }
            s = s.replace(/\t/g, tab);
            at = s.search(cx);
            if (at >= 0) {
...
var ADSAFE = (function() {
    ...
    function reject(object, name) {
        return typeof object !== 'object' || banned[name] || ((typeof name !== 'number' || name < 0) && (typeof name !== 'string' || name.charAt(0) === '_' || name.slice( - 1) === '_' || name.charAt(0) === '-'));
    }
    ...
    function parse_query(text, id) {
        ...
        query = [],
        selector,
        qx = id ? /^\s*(?:([\*\/])|\[\s*([a-z][0-9a-z_\-]*)\s*(?:([!*~|$\^]?\=)\s*([0-9A-Za-z_\-*%&;.\/:!]+)\s*)?\]|#\s*([A-Z]+_[A-Z0-9]+)|:\s*([a-z]+)|([.&_>\+]?)\s*([a-z][0-9a-z\-]*))\s*/: /^\s*(?:([\*\/])|\[\s*([a-z][0-9a-z_\-]*)\s*(?:([!*~|$\^]?\=)\s*([0-9A-Za-z_\-*%&;.\/:!]+)\s*)?\]|#\s*([\-A-Za-z0-9_]+)|:\s*([a-z]+)|([.&_>\+]?)\s*([a-z][0-9a-z\-]*))\s*/;
        do {
            match = qx.exec(text);
            if (!match) {
            ...
            if (match[1]) {
                ...
                if (query.length > 0 || match[5].length <= id.length || match[5].slice(0, id.length) !== id) {
                    return error("ADsafe: Bad query: " + text);
                }
                selector = {
                    op: '#',
    ...
    pecker = {
        ...
        '[^=': function(node) {
            var member = node[name];
            return typeof member === 'string' && member.slice(0, member.length) === value;
        },
        '[$=': function(node) {
            var member = node[name];
            return typeof member === 'string' && member.slice( - member.length) === value;
        },
        '[*=': function(node) {
            var member = node[name];
            return typeof member === 'string' && member.slice.indexOf(value) >= 0;
        },
        '[~=': function(node) {
            var member = node[name];
            return typeof member === 'string' && (' ' + member + ' ').slice.indexOf(' ' + value + ' ') >= 0;
    ...
    function make_root(root, id) {
        ...
        dom_event = function(e) {
            ...
            if (the_actual_event.cancelBubble && the_actual_event.stopPropagation) {
            ...
            if (the_target['___ on ___'] && the_target['___ on ___'][the_event.type]) {
        ...
        Bunch.prototype = {
            append: function(appendage) {
                ...
                if (appendage instanceof Array) {
                    ...
                    for (i = 0; i < b.length; i += 1) {
                        node = b[i];
                        for (j = 0; j < rep.length; j += 1) {
                            node.appendChild(flag ? rep[j].cloneNode(true) : rep[j]);
                        }
                        flag = true;
                    }
                }
                ...
                for (i = 0; i < b.length; i += 1) {
                    ...
                    if (node.tagName) {
                        if (name !== 'float') {
                            ...
                            a[i] = getStyleObject(node).cssFloat || getStyleObject(node).styleFloat;
                        }
                    }
                }
                return a.length === 1 ? a[0] : a;
            ...
            getValue: function() {
                ...
                for (i = 0; i < b.length; i += 1) {
                    ...
                    if (node.nodeName === '#text') {
                        ...
                        a[i] = node.value;
                        if (a[i] === undefined && node.firstChild && node.firstChild.nodeName === '#text') {
            ...
            on: function(type, func) {
                if (this === this.window || typeof type !== 'string' || typeof func !== 'function') {
                    return error();
                }
                var b = this.___nodes___,
                i,
            ...
            replace: function(replacement) {
                ...
                if (!replacement || replacement.length === 0 || replacement.___nodes___.length === 0) {
                    for (i = 0; i < b.length; i += 1) {
                        node = b[i];
                        purge_event_handlers(node);
                        if (node.parentNode) {
            ...
            style: function(name, value) {
                if (this === this.window || value === undefined || /url/i.test(value)) {
                    return error();
                }
                var b = this.___nodes___,
                i,
            ...
            title: function(value) {
                ...
                if (value instanceof Array) {
                    if (value.length !== b.length) {
                        return error('ADsafe: Array length: ' + b.length + '-' + value.length);
                    }
                    for (i = 0; i < b.length; i += 1) {
                        node = b[i];
                        if (node.tagName) {
...
ADSAFE._intercept(function(id, dom, lib, bunch) {
    lib.cookie = {
        ...
        set: function(value) {
            var d,
            j = JSON.stringify(value).replace(/[=]/g, '\\u003d').replace(/[;]/g, '\\u003b');
            if (j.length < 2000) {
                d = new Date();
